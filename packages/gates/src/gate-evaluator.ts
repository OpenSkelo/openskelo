...
import type { BlockGate, GateResult } from "./types.js";
...
export function evaluateBlockGate(
  ...
  switch (gate.check.type) {
    ...
    case "port_matches": {
      ...
      try {
        const regex = new RegExp(gate.check.pattern);
        if (!regex.test(val)) {
          return { name: gate.name, passed: false, reason: gate.error };
        }
        return { name: gate.name, passed: true };
        ...
        return {
          name: gate.name,
          passed: false,
          reason: `${gate.error} (invalid regex: ${err instanceof Error ? err.message : String(err)})`,
          audit: { gate_type: "port_matches", status: "failed", failure: "invalid_regex", pattern: gate.check.pattern },
        };
      }
    }
